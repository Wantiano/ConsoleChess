...
void ChessModel::pawnPromotion(int x_to, int y_to)
{
    if(y_to != 7 && y_to != 0)
        return;

    Piece* p = table[y_to][x_to];
    int piece = view->alarmForPromotion(p->playerColor());
    ...
    {
    case 1:
        p = new Queen(p->getCoords().first,p->getCoords().second,p->playerColor());
        break;
    case 2:
        p = new Rook(p->getCoords().first,p->getCoords().second,p->playerColor());
        break;
    case 3:
        p = new Bishop(p->getCoords().first,p->getCoords().second,p->playerColor());
        break;
    case 4:
        p = new Knight(p->getCoords().first,p->getCoords().second,p->playerColor());
        break;
    default:
        return;
        break;
...
void ChessModel::step(int x_from, int y_from, int x_to, int y_to)
{
    ...
    if(table[y_to][x_to]->getType() == PieceEnum::PAWN)
        pawnPromotion(x_to,y_to);
